...
static inline bool is_manager(void)
{
    ...
    int i;
    if (ksu_manager_appid != KSU_INVALID_APPID && ksu_manager_appid == uid)
        return true;
    for (i = 0; i < KSU_MAX_MANAGERS; i++) {
        if (ksu_manager_appids[i] != KSU_INVALID_APPID && ksu_manager_appids[i] == uid)
            return true;
    }
    return false;
}
...
static inline bool is_uid_manager(uid_t uid)
{
    ...
    int i;
    if (ksu_manager_appid != KSU_INVALID_APPID && ksu_manager_appid == appid)
        return true;
    for (i = 0; i < KSU_MAX_MANAGERS; i++) {
        if (ksu_manager_appids[i] != KSU_INVALID_APPID && ksu_manager_appids[i] == appid)
            return true;
    }
    return false;
}
...
static inline void ksu_add_manager_appid(uid_t appid)
{
    ...
    for (i = 0; i < KSU_MAX_MANAGERS; i++) {
        if (ksu_manager_appids[i] == appid)
            return;
        if (ksu_manager_appids[i] == KSU_INVALID_APPID) {
